...
always @(posedge clk or negedge reset) begin
    ...
    if (!reset) begin
        ...
        // Manejo de transiciones de estado y contadores
        if (state == PIN_INCORRECTO_1 || state == PIN_INCORRECTO_2) begin
            PIN_INGRESADO <= 16'b0;
            contador_pin  <= 3'b0;
        end
        ...
        if (next_state == ESPERANDO_TARJETA) begin
            ...
            contador_pin  <= 3'b0;
            cont_errores  <= 2'b0;
        end else if (state == ESPERANDO_PIN && contador_pin == 3'b100 && PIN_INGRESADO != PIN_CORRECTO) begin
        ...
        ESPERANDO_PIN: begin
            if (contador_pin == 3'b100) begin
                if (PIN_INGRESADO == PIN_CORRECTO) begin
                    ...
                    case (cont_errores)
                        2'b00: next_state = PIN_INCORRECTO_1;
                        2'b01: next_state = PIN_INCORRECTO_2;
                        2'b10: next_state = BLOQUEADO;
                        default: next_state = BLOQUEADO; 
                    endcase
                end 
            end 
        end
